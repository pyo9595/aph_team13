...
class CuteInterpreter(object):
    ...
    def run_func(self, func_node):
        ...
        if func_node.type is TokenType.PLUS:
            ...
            result.value = int(rhs1.value) + int(rhs2.value)
            return result
        ...
        elif func_node.type is TokenType.NOT: #비교는 타입으로!
            rhs1 = self.run_expr(rhs1)
            if rhs1.type is self.TRUE_NODE.type:
                return self.FALSE_NODE
            elif rhs1.type is self.FALSE_NODE.type:
                return self.TRUE_NODE

        elif func_node.type is TokenType.COND:
            return self.runCond(rhs1)
            ...
            if list_is_null(rhs1): return self.TRUE_NODE # null list 이므로 true
            if rhs1.type is not TokenType.LIST: return self.TRUE_NODE # list 아니라 atom
            if rhs1.type is TokenType.LIST: #list 일 때
                if rhs1.value.type is TokenType.QUOTE: #value가 quote이면
                    if rhs1.value.next is not TokenType.LIST: #next가 리스트가 아니니까
                        return self.TRUE_NODE #atom
            return self.FALSE_NODE
            ...
            if list_is_null(rhs1): return self.TRUE_NODE #null이므로
            return self.FALSE_NODE
    ...
    def runCond(self, node):
        if node.value.type is TokenType.LIST:
            cond = self.run_expr(node.value)
        else:
            cond = node.value

        if cond.type not in [self.TRUE_NODE.type,self.FALSE_NODE.type]:
            print ("Type Error!")

        if cond.type is self.FALSE_NODE.type:
            return self.runCond(node.next)
    ...
    def run_list(self, l_node):
        ...
        if op_code.type in \
                [TokenType.CAR, TokenType.CDR, TokenType.CONS, TokenType.ATOM_Q,
                 ...
                 TokenType.TIMES, TokenType.DIV, TokenType.GT, TokenType.LT, TokenType.EQ,
